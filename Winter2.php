...
// Get products for winter season - specifically the EDP products
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Winter' AND p.category = 'eau de parfum'
    ORDER BY p.name ASC
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Winter EDP Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Winter Fragrances - Eau De Parfum</h1>
