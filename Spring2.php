...
// Get products for spring season - specifically for Spring2 page (IDs 52 and 53)
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Spring' AND (p.id = 52 OR p.id = 53)
");
$stmt->execute();
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Spring Fragrances - Eau De Parfum</h1>
