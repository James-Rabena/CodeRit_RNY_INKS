...
// Get products for autumn season - specifically eau fraiche products
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Autumn' AND p.category = 'eau fraiche'
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Autumn EF Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Autumn Fragrances - Eau De Fraiche</h1>
        ...
        function addToCart(productData) {
            ...
            if (isLoggedIn) {
                ...
                try {
                    ...
                    // Calculate new cart count and update IMMEDIATELY (outside setTimeout)
