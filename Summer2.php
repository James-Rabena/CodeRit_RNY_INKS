...
// Get products for summer season - specifically the EDP products for Summer2 page
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Summer' AND p.id IN (66, 67)
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Summer EDP Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Summer Fragrances - Eau De Parfum</h1>
