...
// Get products for the Autumn season - specifically the EDP products for Autumn2 page
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Autumn' AND p.id IN (39, 40)
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Autumn EDP Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Autumn Fragrances - Eau De Parfum</h1>
            ...
            if (isLoggedIn) {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        // Add animation to cart icon if available
                        const cartIcon = document.querySelector('.fa-shopping-cart');
                        if (cartIcon) {
                            cartIcon.classList.add('fa-pulse');
                            setTimeout(() => {
                                cartIcon.classList.remove('fa-pulse');
                            }, 1000);
                        }
                    } else {
                ...
                try {
                    ...
                    button.innerHTML = originalHTML;
                    button.disabled = false;
                    showNotification('Error', 'Could not add to cart', 'danger');
                }
            }
        }
