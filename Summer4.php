...
// Get the season data
$stmt = $conn->prepare("SELECT * FROM seasons WHERE name = 'Summer'");
$stmt->execute();
$season = $stmt->get_result()->fetch_assoc();
$stmt->close();

// Get products for summer season - specifically the EDC products for Summer4 page
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Summer' AND p.id IN (70, 71)
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Summer EDC Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Summer Fragrances - Eau De Cologne</h1>
    <p class="text-center modern-subheading">Discover our refreshing collection perfect for the warm season.</p>
