...
// Get products for spring season - specifically eau de toilette products (IDs 54 and 55)
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Spring' AND p.category = 'eau de toilette'
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Spring Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Spring Fragrances - Eau De toilette</h1>
