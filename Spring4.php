...
// Get the season data
$stmt = $conn->prepare("SELECT * FROM seasons WHERE name = 'Spring'");
$stmt->execute();
$season = $stmt->get_result()->fetch_assoc();
$stmt->close();

// Get products for spring season - specifically eau de cologne products (IDs 56 and 57)
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Spring' AND p.category = 'eau de cologne'
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Spring EDC Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Spring Fragrances - Eau De Cologne</h1>
    <p class="text-center modern-subheading">Embrace the season of renewal with our fresh and light eau de cologne fragrances.</p>
