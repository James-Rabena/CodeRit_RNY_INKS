...
// Get products for summer season - specifically the EDT products for Summer3 page
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Summer' AND p.id IN (68, 69)
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Summer EDT Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Summer Fragrances - Eau De toilette</h1>
