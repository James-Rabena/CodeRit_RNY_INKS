...
// Get products for spring season - specifically eau fraiche products (IDs 58 and 59)
$stmt = $conn->prepare("
    SELECT p.* FROM products p
    JOIN product_seasons ps ON p.id = ps.product_id
    JOIN seasons s ON ps.season_id = s.id
    WHERE s.name = 'Spring' AND p.category = 'eau fraiche'
");
$stmt->execute();
...
<head>
    ...
    <title>Fragrance Fusion - Spring EF Collection</title>
...
<div class="container mt-5">
    <h1 class="text-center modern-heading">Spring Fragrances - Eau De Fraiche</h1>
